...
def generar_grafico_y_guardar_imagen(archivo_csv, tamanos, carpeta_destino, nombre_imagen):
    ...
    # Gráfica de Speedup vs. Procesadores para cada tamaño
    for tamano in tamanos:
        ...
        plt.scatter(data_tamano['Procesadores'], data_tamano['Speedup'], label=f'Tamaño {tamano}')
        plt.plot(data_tamano['Procesadores'], data_tamano['Speedup'], linestyle='-', marker='o', markersize=5)

    plt.xlabel('Número de Procesadores')
    plt.ylabel('Speedup')

    # Título y subtítulo más concisos
    plt.title('Speedup vs. Procesadores', y=1.05, fontsize=16)
...
for archivo in archivos_csv:
    nombre_imagen = f'Speedup vs. Procesadores_{obtener_texto_desde_nombre(archivo[:-4])}.png'  # Generar un nombre único para cada imagen
